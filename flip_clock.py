...
MY_VIP_LIST = [
    ...
    {"zh": "大 阪", "en": "Osaka", "tz": "Asia/Tokyo", "q": "Osaka", "lat": 34.69, "lon": 135.50, "vip": True, "img": "https://images.unsplash.com/photo-1590559899731-a382839e5549?w=1200&q=80"},
    {"zh": "札 幌", "en": "Sapporo", "tz": "Asia/Tokyo", "q": "Sapporo", "lat": 43.06, "lon": 141.35, "vip": True, "img": "https://www.sapporo.travel/cms/wp-content/uploads/2020/10/moiwa-yakei-1200x600-1.jpg"},
    {"zh": "上 海", "en": "Shanghai", "tz": "Asia/Shanghai", "q": "Shanghai", "lat": 31.23, "lon": 121.47, "vip": True, "img": "https://images.unsplash.com/photo-1548919973-5cfe5d4fc474?w=1200&q=80"},
    ...
    {"zh": "法蘭克福", "en": "Frankfurt", "tz": "Europe/Berlin", "q": "Frankfurt", "lat": 50.11, "lon": 8.68, "vip": True, "img": "https://images.unsplash.com/photo-1541410965313-d53b3c16ef17?w=1200&q=80"},
...
def render_flip_clock(api_key, target_idx=0):
    ...
    <style>
        * {{ box-sizing: border-box; }}
        body {{ 
            background: #0e1117; margin: 0; display: flex; justify-content: center; 
            font-family: "Impact", sans-serif; color: white; overflow: hidden; 
        }}
        
        /* 使用 Viewport Wrapper 確保在手機上自動縮小不溢出 */
        .viewport-wrapper {{
            width: 100vw;
            display: flex;
        ...
        /* 響應式縮放：當手機螢幕窄於 440px 時整體縮小 */
        @media (max-width: 440px) {{ .main-container {{ transform: scale(0.88); }} }}
        @media (max-width: 380px) {{ .main-container {{ transform: scale(0.8); }} }}
        @media (max-width: 330px) {{ .main-container {{ transform: scale(0.72); }} }}

        .brand-text {{ color: #555; font-size: 14px; letter-spacing: 6px; font-weight: bold; margin-bottom: 5px; }}
        .clock-board {{ width: 100%; display: flex; flex-direction: column; gap: 10px; cursor: pointer; }}
        .card-row {{ display: flex; width: 100%; gap: 8px; justify-content: center; align-items: center; }}
        .card {{ background: #1a1a1a; border-radius: 8px; position: relative; overflow: hidden; perspective: 1000px; }}
        
        .info-card {{ flex: 1; height: 75px; font-size: 32px; display: flex; align-items: center; justify-content: center; }}
        .weather-card {{ background: #141414; color: #888; font-size: 20px; font-family: sans-serif; height: 55px; }}
        .time-card {{ width: 90px; height: 130px; font-size: 100px; }}
        ...
        .city-banner {{ width: 100%; height: 190px; border-radius: 12px; background-size: cover; background-position: center; transition: 1s; position: relative; }}
        .vignette {{ position: absolute; inset: 0; background: radial-gradient(circle, transparent 30%, rgba(0,0,0,0.8) 100%); }}
        .map-overlay-btn {{ position: absolute; bottom: 0; left: 0; width: 70px; height: 70px; cursor: pointer; z-index: 50; }}
    </style>
    ...
    <script>
        ...
        async function updateWeather() {{
            ...
            }} catch(e) {{ smartUpdate('w_desc', 'Loading...'); }}
        }}
